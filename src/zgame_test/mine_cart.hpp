...
class MineCart : public mz::Initialisable, public mz::Updatable
{
struct NextRailInfo;

public:
    void place_on_rail(mz::Entity e, Dir init_direction, Rail rail, mz::Vector2f position) {
        ...
        if (auto* t = e.write<mz::Transform>()) {
            t->set_position(position);
        }
    }

private:
    void start(mz::Entity e) override {
        place_on_rail(e, Dir::N, Rail(Type::NorthSouth), { 0, 32 });
    }

    void update(mz::Entity e, float dt) override {
        m_current_time += dt;
        ...
        mz::Vector2f current_pos = m_last_position.lerp(m_next_position, t);
    ...
    void get_next_rail_info(mz::Entity e) {
        if (const auto* rail_map = e.read<RailMap>()) {
            mz::Vector2i grid_position = GridUtils::pixel_to_grid(
    ...
    std::optional<NextRailInfo> get_next_rail(const RailMap* rail_map, mz::Vector2i grid_position) {
        ...
        mz::Vector2i next_grid_pos = grid_position + direction_to_offset(exit_dir);
    ...
    mz::Vector2i direction_to_offset(Dir dir) const {
        switch (dir) {
            case Dir::N:    return mz::Vector2i(+0, +1);
            case Dir::E:    return mz::Vector2i(+1, +0);
            case Dir::S:    return mz::Vector2i(+0, -1);
            case Dir::W:    return mz::Vector2i(-1, +0);
            default:        return mz::Vector2i(+0, +0);
        }
    }
...
private:
    struct NextRailInfo 
    {
        Rail rail;
        Dir travel_dir;
        mz::Vector2i next_grid_pos;
    };

private:
    mz::Vector2f m_next_position;
    mz::Vector2f m_last_position;
